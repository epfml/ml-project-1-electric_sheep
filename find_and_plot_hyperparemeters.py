...
d = len(x_features) - 1
categoricals = np.full(d, True)
categoricals[-18:] = False # All the scalar features
...
def all_x_processing(x, categoricals, degree):

    x_scalar = x[:, ~categoricals]
...
for degree in degrees:
    ...
    for lambda_ in lambdas:
        ...
        tx = all_x_processing(x, categoricals, degree)
        tx_train = tx[:N_train]
        tx_test = tx[N_train:]
...
opt_degree = degrees[opt_idx // len(lambdas)]
opt_lambda = lambdas[opt_idx % len(lambdas)]
print(f"\nBest degree : {opt_degree}, best lambda : {opt_lambda}") # we can do modulo to find the hpp

x = np.concatenate((x_train, x_test), axis=0)
tx = all_x_processing(x, categoricals, opt_degree)
tx_test = tx[N_train:]
